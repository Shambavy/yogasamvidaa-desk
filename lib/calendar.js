...
export async function syncClassToCalendar(accessToken, cls) {
  ...
  const meetLink = res.data.conferenceData &&
    res.data.conferenceData.entryPoints &&
    res.data.conferenceData.entryPoints.find(function(e) {
      return e.entryPointType === "video"
    })
  return { eventId: res.data.id, meetLink: meetLink ? meetLink.uri : null }
}
...
export async function createAppointmentEvent(accessToken, appt) {
  ...
  const withMeet = appt.mode && appt.mode.includes("Online")
  const event = {
